...
def generate_cd_negatives(
    ...
    *,
    ...
    steps: int | None = None,
    # ADDED: Arguments to support Persistent Contrastive Divergence (PCD)
    ...
    with torch.no_grad():
        result = solver.simulate(
            ...
            device=device,
            record_states=False,
            # Pass persistent states for MCMC continuation
            initial_states=initial_states,
            initial_mask=initial_mask,
        )
        # The result object contains the final states and the mask for PCD
...
def train(cfg):
    ...
    # ADDED: Buffers for Persistent Contrastive Divergence (PCD)
    pcd_negatives = None
    pcd_init_mask = None
    ...
    for epoch in range(cfg.epochs):
        ...
        for batch in pbar:
            ...
            # Generate negative samples using the persistent buffer for PCD
            result = generate_cd_negatives(
                model,
                dataset_tensor,
                cfg.batch_size,
                device,
                ...
                # Pass the persistent states from the previous iteration
                initial_states=pcd_negatives,
                initial_mask=pcd_init_mask,
            )

            x_neg = result.states.to(device)
            loss_cd = contrastive_divergence_loss(model, x_data, x_neg)

            # Store the final states and mask for the *next* iteration (PCD)
            # Detach and clone to avoid memory leaks during graph creation
            pcd_negatives = result.states.detach().clone()
            pcd_init_mask = result.init_mask.detach().clone()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Main Energy Matching training with FM + CD objectives."
    )
    parser.add_argument(
        "--resume", type=str, default="energy_checkpoints/warmup/energy_model_epoch_100.pt"
    )
    ...
    parser.add_argument(
        "--checkpoint-path",
        type=str,
        default="energy_checkpoints/cd_annealing/main_training_final.pt",
    )
    parser.add_argument(
        "--loss-plot", type=str, default="energy_checkpoints/cd_annealing/main_training_losses.png"
    )
    parser.add_argument(
        "--log-path", type=str, default="energy_checkpoints/cd_annealing/main_training_log.json"
    )
